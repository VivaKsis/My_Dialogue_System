...
[CustomPropertyDrawer(typeof(AllFramesSelectionAttribute))]
public class AllFramesSelectionAllFramesSelectionAttributeDrawer : PropertyDrawer
{
		...
		if (GUI.Button(buttonRect, buttonLabel))
		{
			...
			for (int i = 0; i < guids.Length; i++)
			{
				...
				if (frame != null && (frame.GetComponent<FrameImage>() != null ||
									  frame.GetComponent<FrameText>() != null ||
									  frame.GetComponent<FrameAnswers>() != null))
				{
					GUIContent content = new GUIContent(frame.name);
					string[] nameParts = frame.name.Split(' ');
					if (nameParts.Length > 1) content.text = nameParts[0] + "/" + frame.name.Substring(nameParts[0].Length + 1);
