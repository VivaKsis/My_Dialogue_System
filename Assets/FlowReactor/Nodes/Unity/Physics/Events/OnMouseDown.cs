...
{
	[NodeAttributes("Unity/Physics/Events", "OnMouseDown is called when the user has pressed the mouse button while over the Collider.", "eventNodeColor", 1, NodeAttributes.NodeType.Event)]
	public class OnMouseDown : Node
	{
		...
		public override void OnInitialize(FlowReactorComponent _flowReactor)
		{
			...
			if (collider.Value != null)
			{
				...
				if (_go != null)
				{
					...
					if (_component != null)
					{
						_component.onMouseDownNodes.Add(this);
					}
					else
					{
						_go.AddComponent<MouseColliderListener>().onMouseDownNodes.Add(this);
					}
				}
			}
		}
