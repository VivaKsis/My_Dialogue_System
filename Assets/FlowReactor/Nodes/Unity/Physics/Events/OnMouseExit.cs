...
{
	[NodeAttributes("Unity/Physics/Events", "Called when the mouse is not any longer over the Collider.", "eventNodeColor", 1, NodeAttributes.NodeType.Event)]
	public class OnMouseExit : Node
	{
		...
		public override void OnInitialize(FlowReactorComponent _flowReactor)
		{
			...
			if (collider.Value != null)
			{
				...
				if (_go != null)
				{
					...
					if (_component != null)
					{
						_component.onMouseExitNodes.Add(this);
					}
					else
					{
						_go.AddComponent<MouseColliderListener>().onMouseExitNodes.Add(this);
					}
				}
			}
		}
